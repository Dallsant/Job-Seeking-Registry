...
export class ResponseManager implements Provider<any>{
  ...
  validateRequest(data: any, verifyObject: any) {
    Object.keys(verifyObject).map((keyProperty, index) => {
      if (!data.hasOwnProperty(keyProperty)) {
        ...
        if (verifyObject[keyProperty] !== 'required') {
          try {
            this.caseTypeVars(verifyObject[keyProperty], data, keyProperty);
          } catch (error) {
            throw ('Error');
          }
        }
